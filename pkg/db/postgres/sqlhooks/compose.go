...
func (c composed) Before(ctx context.Context, query string, args ...any) (context.Context, error) {
	var errors []error
	for _, hook := range c {
...
func (c composed) After(ctx context.Context, query string, args ...any) (context.Context, error) {
	var errors []error
	for _, hook := range c {
		var err error
...
func (c composed) OnError(ctx context.Context, cause error, query string, args ...any) error {
	var errs []error
	for _, hook := range c {
		if onErrorer, ok := hook.(OnErrorer); ok {
			if err := onErrorer.OnError(ctx, cause, query, args...); err != nil && !errors.Is(err, cause) {
				errs = append(errs, err)
			}
		}
	}
	return wrapErrors(cause, errs)
}
